...
| Command | Purpose |
|---------|---------|
| `./scripts/ship-remote.sh` | **The one ship command, run from this Mac.** Ships on `macmini-remote` via `ship-on-mini.sh`, verifies `IN_BETA_TESTING`, records the build (`record-build.sh`), pushes |
